...
impl Connection {
    ...
    pub fn connect(&mut self, communication_zone: usize) -> Result<(), &'static str> {
        ...
        debug!(
            "The memory address of the IVC Info: VA: 0x{:x}, IPA: 0x{:x}",
            vpa_ivcinfo.as_usize(),
            pa_ivcinfo.as_usize()
        );

        ivc_hvc_call(
            HVISOR_HC_IVC_INFO as u32,
            pa_ivcinfo.as_usize(),
            HVISOR_HC_IVC_INFO_SIZE,
        );
    ...
    pub fn send_message(&mut self, message: &str) -> Result<(), &'static str> {
        ...
        let vpa_share_memory_zone1 = phys_to_virt(PhysAddr::from(
            (ivc_info.ivc_shmem_ipas[0] + offset) as usize,
        ));
